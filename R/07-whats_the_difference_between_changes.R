...
whats_the_difference_between_changes <- function(df,cluster1, cluster2,         
                                                  signif = 0.05) {
        ...
        clusters = rbind(cluster1, cluster2) %>%
                select(uid, cluster, absolute)
        clusters = split(clusters, clusters$uid)
        for (i in names(clusters)){
                p = try(t.test(data = clusters[[i]], absolute~cluster)$p.value)
                if(class(p) != "try-error"){
                        clusters[[i]] = data.frame(uid = unique(clusters[[i]]$uid), 
                                                   cluster1_mean = mean(clusters[[i]]$absolute[clusters[[i]]$cluster == 1]),
                                                   cluster2_mean = mean(clusters[[i]]$absolute[clusters[[i]]$cluster == 2]),
                                                   p = p)
                }else{
                        clusters[[i]] = NULL
                }
